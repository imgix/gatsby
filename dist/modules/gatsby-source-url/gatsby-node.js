...
exports.createResolvers = function (_a, _options) {
    var _b;
    var createResolversCb = _a.createResolvers, cache = _a.cache;
    return pipeable_1.pipe(Do_1.Do(E.either)
        .bind('options', pipeable_1.pipe(publicTypes_1.GatsbySourceUrlOptions.decode(_options), E.mapLeft(function () {
        return new Error("[@imgix/gatsby] The plugin config is not in the correct format.");
    }), E.chain(function (options) {
        if (options.sourceType === 'webProxy' &&
            (options.secureURLToken == null ||
                options.secureURLToken.trim() === '')) {
            return E.left(new Error("[@imgix/gatsby] the plugin option 'secureURLToken' is required when sourceType is 'webProxy'."));
        }
        ...
        .bind('packageVersion', pipeable_1.pipe((_b = read_pkg_up_1.default.sync({ cwd: __dirname })) === null || _b === void 0 ? void 0 : _b.packageJson.version, E.fromNullable(new Error('Could not read package version.'))))
        .doL(function (_a) {
